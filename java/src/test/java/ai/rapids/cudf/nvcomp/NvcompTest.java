...
public class NvcompTest {
  ...
  @Test
  void testLZ4RoundTripViaGenericDecompressorSync() {
    lz4RoundTrip(false, false);
  }

  @Test
  void testLZ4RoundTripViaGenericDecompressorAsync() {
    lz4RoundTrip(true, false);
  }

  @Test
  void testLZ4RoundTripViaLZ4DecompressorSync() {
    lz4RoundTrip(false, true);
  }

  @Test
  void testLZ4RoundTripViaLZ4DecompressorAsync() {
    lz4RoundTrip(true, true);
  }
  ...
  private void lz4RoundTrip(boolean useAsync, boolean useLz4Decompressor) {
    final Cuda.Stream stream = Cuda.DEFAULT_STREAM;
    final int chunkSize = 64 * 1024;
    ...
    try (ColumnVector v = ColumnVector.fromLongs(data)) {
      ...
      Assertions.assertTrue(Decompressor.isLZ4Data(compressedBuffer, stream));

      try (Decompressor.Metadata metadata =
               Decompressor.getMetadata(compressedBuffer, stream)) {
        ...
        try (LZ4Decompressor.Configuration decompressConf =
          ...
          Assertions.assertEquals(tempSize, metadata.getTempSize());
          Assertions.assertEquals(tempSize, Decompressor.getTempSize(metadata));
          ...
          Assertions.assertEquals(outSize, metadata.getOutputSize());
          Assertions.assertEquals(outSize, Decompressor.getOutputSize(metadata));
          Assertions.assertEquals(inputBuffer.getLength(), outSize);

          uncompressedBuffer = DeviceMemoryBuffer.allocate(outSize);

          if (useLz4Decompressor) {
